...
def main() -> None:
    ...
    # 16. Workspace resolution delegated to _workspace_resolve.py helper
    _ws_resolve = REPO / "skills" / "agent-registry" / "scripts" / "_workspace_resolve.py"
    ws = _ws_resolve.read_text() if _ws_resolve.exists() else svc
    expect(
        "SUTANDO_WORKSPACE" in ws,
        "registry-service.py: must respect SUTANDO_WORKSPACE env var for workspace path",
    )
    expect(
        ".sutando/workspace" in ws,
