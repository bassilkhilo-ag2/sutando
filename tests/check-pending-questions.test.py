...
"""
    result = _run_gwq(content)
    if len(result) == 1 and "Section without status" in result[0].get("title", ""):
        ok("(gwq-e) no Status field → included as open (free-form)")
    else:
        fail("(gwq-e) no Status field → included as open", f"got {result}")
